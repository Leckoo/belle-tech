...
function Stepper({
  ...
  const unregisterTrigger = React.useCallback((node: HTMLButtonElement) => {
    setTriggerNodes((prev) => prev.filter((currentNode) => currentNode !== node));
  }, []);
...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  const setButtonRef = React.useCallback(
    (node: HTMLButtonElement | null) => {
      if (nodeRef.current && nodeRef.current !== node) {
        unregisterTrigger(nodeRef.current);
      }
  ...
  const myIdx = React.useMemo(() => {
    ...
    return triggerNodes.findIndex((node) => node === triggerNode);
...
function StepperIndicator({ children, className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'relative flex items-center justify-center size-6 shrink-0 overflow-hidden rounded-full border-background bg-accent text-accent-foreground text-xs data-[state=completed]:bg-primary data-[state=completed]:text-primary-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground',
        className,
      )}
    >
